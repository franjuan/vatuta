...
def _display_stats(dm: QdrantDocumentManager) -> None:
    """Display KB stats before answering."""
...
def _init_sources(state: State, entity_manager: EntityManager) -> List[Any]:
    """Initialize active sources matching config."""
...
def _display_router_cot(router_cot_data: Dict[str, Any]) -> None:
    """Display the router chain of thought reasoning."""
...
def _display_cot(result: Dict[str, Any], show_cot: bool, verbose: bool) -> None:
    """Display the reasoning and router chain of thought if requested."""
    if show_cot:
        if result.get("router_cot"):
            _display_router_cot(result["router_cot"])
...
def _display_sources(result: Dict[str, Any]) -> None:
    """Display retrieved sources using rich tables."""
...
def ask(
    ...
    max_tokens: Optional[int] = typer.Option(None, "--max-tokens", help="Maximum number of tokens for LLM generation"),
    ...
    if max_tokens is not None:
        generator_backend = state.config.rag.generator_backend
        if generator_backend in state.config.rag.llm_backends:
            state.config.rag.llm_backends[generator_backend].max_tokens = max_tokens
