...
2. **Chunk model** (4 distinct chunk types)
    ...
    - **Comment Chunk(s)**:
      ...
      - **Strategies**:
        - **Size/Count**: Merges comments into a single chunk until `chunk_max_size_chars` (capped by embedding model
          limit) or `chunk_max_comments` is reached.
...
4. **Observability**
    - Vatuta uses Prometheus for tracking API performance and ingestion quality metrics.
    - See [Observability & Metrics](../observability.md) for full details on API trackers, chunk sizing, and split reasons.
